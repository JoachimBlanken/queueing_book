...
Finding general conditions when to stop is not directly easy, but a pragmatic approach is to simple: stop at some (large) number $N$ such that $\pi(k)\ll \pi(0)$ for $k>N$ and is steadily decreasing.\footnote{
question, why should it decrease monotonically after some, large, $N$?}
...
services, i.e., the $M/M^Y/1$ queue. Constructing a recursion for the
